...
def main(argv=None):
    ...
    experiments = [
        ("IF", build_if),
        ("TimesNet", lambda: TimesNetModel()),
        ("Informer", lambda: InformerModel()),
        ("TFT", lambda: TFTModel()),
    ]

    results = {}
    for name, factory in experiments:
        val_loss, metrics = run_experiment(name, factory, train_loader, val_loader, test_loader, device, scaler_raw)
